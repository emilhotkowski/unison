  ...
  use LoopState.latestTypecheckedFile >>= \case
    ...
    Just uf -> do
      ...
      let sr :: SlurpResult v
          sr = Slurp.slurpFile uf requestedVars Slurp.UpdateOp slurpCheckNames
...
data Structure v a = Structure
  { structureDecls :: [UpsertDecl v a],
    structureTerms :: [UpsertTerm v],
    -- Look up the type of a term in the structure. Only valid for terms in the structure (old or new).
    -- FIXME this should be computable from structureTerms, no?
    structureTermType :: TermReference -> Type v a
  }

structureIsEmpty :: Structure v a -> Bool
...
structureToUpdates :: forall a m v. Var v => Branch0 m -> Structure v a -> [(Path, Branch0 m -> Branch0 m)]
structureToUpdates branch structure =
  let declUpsertToOps :: UpsertDecl v a -> [(Path, Branch0 m -> Branch0 m)]
      declUpsertToOps = \case
        Add AddDecl {declref = Declref {decl, ref}, var} ->
          ( BranchUtil.makeAddTypeName
          ...
          DeclUpsertOpUpdate DeclUpdateOp {ref0, ref1, var} ->
            concat
              [ let f (name, metadata) =
                      [ BranchUtil.makeDeleteTypeName name ref0,
                        BranchUtil.makeAddTypeName name (Reference.fromId ref1) metadata
                      ]
                 in foldMap f (getTypeInfo ref0),
      ...
      termUpsertToOps :: UpsertTerm v -> [(Path, Branch0 m -> Branch0 m)]
      termUpsertToOps = \case
        Add AddTerm {termref, var} ->
          [ BranchUtil.makeAddTermName
              (Path.splitFromName (Name.unsafeFromVar var))
              (Referent.fromTermReferenceId (untagStoredTerm termref ^. #ref))
              (if storedTermIsTestWatch termref then isTestMetadata else Metadata.empty)
          ]
        Update UpdateTerm {update, var} -> do
          let ref0 =
                Referent.Ref case update of
                  Explicit Upd1 {before} -> untagStoredTerm before
                  Implicit Upd1 {before} -> Reference.fromId (untagStoredTerm before ^. #ref)
          let ref1 = Referent.fromTermReferenceId (untagStoredTerm (updAfter update) ^. #ref)
          (name, oldMetadata) <- getTermInfo ref0
          -- FIXME (existing) bug? shouldn't we delete isTest when updating from test to non-test?
          let newMetadata =
                let wasTestBefore =
                      case update of
                        Explicit Upd1 {before} -> storedTermIsTestWatch before
                        Implicit Upd1 {before} -> storedTermIsTestWatch before
                    isTestNow = storedTermIsTestWatch (updAfter update)
                 in oldMetadata
                      & case (wasTestBefore, isTestNow) of
                        (False, True) -> Metadata.merge isTestMetadata
                        (True, False) -> Map.delete isTestType
                        _ -> id
          [ BranchUtil.makeDeleteTermName name ref0,
            BranchUtil.makeAddTermName name ref1 newMetadata
            ]
   in concat
  ...
  where
    ...
    isTestType :: Metadata.Type
    isTestValue :: Metadata.Value
    (isTestType, isTestValue) =
      isTest
...
structureToVars :: Structure v a -> Set v
structureToVars = undefined

structureTermEdits :: Structure v a -> [(TermReference, TermReference)]
structureTermEdits structure =
  mapMaybe termUpsertUpdate (structureTerms structure)

structureTypeEdits :: Structure v a -> [(TypeReference, TypeReference)]
structureTypeEdits structure =
  mapMaybe declUpsertUpdate (structureDecls structure)
...
type UpsertDecl v a =
  Upsert (AddDecl v a) (UpdateDecl v a)

declUpsertUpdate :: UpsertDecl v a -> Maybe (TypeReference, TypeReference)
declUpsertUpdate = \case
  Add _ -> Nothing
...
-- FIXME document
declUpsertsToDeclsByVar ::
  forall a v.
  Ord v =>
  [UpsertDecl v a] ->
  (Map v (TypeReferenceId, DD.EffectDeclaration v a), Map v (TypeReferenceId, DD.DataDeclaration v a))
declUpsertsToDeclsByVar =
  ...
  where
    untangle ::
      UpsertDecl v a ->
      Either
        (v, (TypeReferenceId, DD.EffectDeclaration v a))
        (v, (TypeReferenceId, DD.DataDeclaration v a))
    untangle = \case
      Add AddDecl {declref, var} -> go var declref
      Update UpdateDecl {update, var} -> go var (updAfter update)
      where
        go var Declref {decl, ref} =
          case decl of
...
type DeclrefId v a =
  GDeclref v a TypeReferenceId

data GDeclref v a r = Declref
  { decl :: Decl v a,
    ref :: r
  }
  deriving stock (Generic)
...
type UpsertTerm v =
  Upsert (AddTerm v) (UpdateTerm v)

data AddTerm v = AddTerm
  { termref :: StoredTerm (TermrefId v Ann),
    var :: v
  }

data UpdateTerm v = UpdateTerm
  { update :: Upd (StoredTerm TermReference) (StoredTerm (TermrefId v Ann)),
    var :: v
  }

data GTermref v a r = Termref
  { term :: Term v a,
    ref :: r
  }
  deriving stock (Generic)

-- | A term that we store in the codebase. It's either a normal (non-test-watch) term, or a test watch.
data StoredTerm a
  = NormalTerm a
  | TestWatch a

-- | Tag a thing as a stored term, per its watch kind.
tagStoredTerm :: a -> Maybe WK.WatchKind -> Maybe (StoredTerm a)
tagStoredTerm thing = \case
  Nothing -> Just (NormalTerm thing)
  Just WK.TestWatch -> Just (TestWatch thing)
  _ -> Nothing

-- | Forget whether a stored is a normal term or a test watch.
untagStoredTerm :: StoredTerm a -> a
untagStoredTerm = \case
  NormalTerm x -> x
  TestWatch x -> x

-- | Is this stored term a test watch?
storedTermIsTestWatch :: StoredTerm a -> Bool
storedTermIsTestWatch = \case
  NormalTerm _ -> False
  TestWatch _ -> True

watchKindToStoredTerm :: Maybe WK.WatchKind -> Maybe (a -> StoredTerm a)
watchKindToStoredTerm = \case
  Nothing -> Just NormalTerm

type TermrefId v a =
  GTermref v a TermReferenceId

termUpsertUpdate :: UpsertTerm v -> Maybe (TermReference, TermReference)
termUpsertUpdate = \case
  Add _ -> Nothing
  Update UpdateTerm {update} ->
    Just case update of
      Explicit Upd1 {before, after} -> (untagStoredTerm before, taggedTermrefIdToRef after)
      Implicit Upd1 {before, after} -> (taggedTermrefIdToRef before, taggedTermrefIdToRef after)
  where
    taggedTermrefIdToRef :: StoredTerm (TermrefId v Ann) -> TermReference
    taggedTermrefIdToRef =
      Reference.fromId . view #ref . untagStoredTerm
...
data SlurpResult2 v = SlurpResult2
  { slurp :: SlurpResult v,
    -- | The mapping from old decl to new decl, for all decl updates.
    declMapping :: Map TypeReference TypeReferenceId,
    -- | Get the the old (pre-update) reference for a decl being updated.
    declRef0 :: v -> TypeReference,
    -- | The mapping from old term to new term, for all term updates.
    termMapping :: Map TermReference TermReferenceId,
    -- | Get the the old (pre-update) reference for a term being updated.
    termRef0 :: v -> TermReference
  }
  deriving stock (Generic)
...
-- the terms in the original unison file
--
-- Hmm... that seems a bit difficult to tackle right now. Going to do an easier thing - pulling a couple fields out of
-- SlurpResult2. Done.
--
-- More efficient implementation of declRef1, no need to construct names from unison file.
--
-- Clean up SlurpResult2 innards, document fields.
--
-- Delete SlurpForUpdate.
--
-- Document StoredTerm and friends.
--
-- Finish termUpsertUpdate implementation.
--
-- Add 'a' type var to a bunch of things (Structure, UpsertDecl, UpsertTerm)
--
-- Pull out declUpdatesToDeclsByVar to top-level.
--
-- 22/03/21
--
-- Simplify classifying stored terms.
--
...
oink ::
  ...
  Set v ->
  TypecheckedUnisonFile v Ann ->
  Action' m v (Structure v Ann)
oink selection unisonFile0 = do
  currentPath' <- use LoopState.currentPath
  currentBranch' <- getAt currentPath'
  let currentBranch0 = Branch.head currentBranch'
  let allNames = Branch.toNames currentBranch0
  let declNames = Names.namesForReference allNames . Reference.fromId
  let termNames = Names.namesForReferent allNames . Referent.fromTermReferenceId

  -- Helper function that determine whether a term reference appears to be a test, per whether it contains the isTest
  -- metadata type/value pair.
  let termIsTest :: TermReference -> Bool
      termIsTest ref =
        let (isTestType, isTestValue) = isTest
         in Metadata.hasMetadata'
              (Referent.fromTermReference ref)
              isTestType
              isTestValue
  ...
  pure
    Structure
      { structureDecls = declUpserts,
        structureTerms = termUpserts
      }
  where
    ...
    typecheck :: UF.UnisonFile v Ann -> Action' m v (Maybe (TypecheckedUnisonFile v Ann))
    typecheck file = do
      result <- eval (TypecheckFile file [])
      pure case runIdentity (Result.toMaybe result) of
...
--    explicitly)
-- 2. New cycles could be formed (e.g. if we're updating Ping to *now* refer to Pong, which *already* referred to Ping)
--    (this isn't implemented yet).
--
-- For example, consider the following codebase.
--
-- @
-- structural type Ping = Ping Nat Pong
-- structural type Pong = Pong Int Ping
-- @
--
-- If we pull the following into a unison file and update,
--
-- @
-- structural type Ping = Ping Nat Nat Pong
-- @
--
-- then by "hydrating" the decls in the unison file, we'll end up with something like this instead.
--
-- @
-- structural type Ping = Ping Nat Nat ℍ0
-- structural type ℍ0 = Pong Int Ping
-- @
--
...
hydrateUnisonFileDecls loadDeclComponent declNames slurp@DeclSlurp {declsById1, updates, varsById1} = do
  ...
  --
  --   In codebase:
  --     unique type Ping = Ping (Maybe Pong)
  --     unique type Pong = Pong (Maybe Ping)
  --
  --   In scratch.u:
  --     -- An update to a member of a component
  --     unique type Ping = Ping (Maybe Pong) Nat
  --     -- A random addition
  --     unique type Hi = Hi Nat
  --
  --   Rough representation of decl, for the purpose of this example:
  --
  --     #Ping1 = [[Maybe #Pong0, Nat]]
  --     ^^^^^^   ^^^^^^^^^^^^^^^^^^^^^
  --     hash     list of constructors, each of which is a list of types

  -- Running example:
  --
  --   #Pong0 => [[Maybe #Ping0]]
  implicitDecls <- loadExtraObjects loadDeclComponent declNames (Map.map fst updates)

  pure $
    fromMaybe (declSlurpToDeclUpserts slurp) do
      guard (not (Map.null implicitDecls))
      ...
      let -- Classify this hashed decl as either an add, an explicit update, or an implicit update.
          --
          -- Running example:
          --
          --   ("v0", #Hi0, [[Nat]]) ->
          ...
          --       "Hi"
          --
          --   ("v1", #Ping2, [[Maybe #Pong1, Nat]]) ->
          --     UpdateDecl
          --       (Explicit
          --         (Upd1
          --           (Declref [[Maybe #Pong0]] #Ping0)
          --           (Declref [[Maybe #Pong1, Nat]] #Ping2))
          --       "Ping"
          --
          --   ("v2", #Pong1, [[Maybe #Ping2]]) ->
          --     UpdateDecl
          --       (Implicit
          --         (Upd1
          --           (Declref [[Maybe #Ping0]] #Pong0)
          --           (Declref [[Maybe #Ping2]] #Pong1)))
          --       "v2"
          ...
          classify (randomName, ref1, decl1)
            | isExtraDecl =
              let ref0 = randomNameToOldRef Map.! randomName
                  before = Declref {decl = implicitDecls Map.! ref0, ref = ref0}
